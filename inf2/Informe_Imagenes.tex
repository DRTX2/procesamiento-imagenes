...
Este control establece la dimensión de la región cuadrada utilizada en la compresión, de manera que cada bloque de $N \times N$ píxeles se sustituye por un único valor promedio. En términos prácticos, este parámetro controla el compromiso entre reducción de datos y conservación de detalle espacial: al incrementar $N$, disminuyen la variabilidad local y el ruido de alta frecuencia, pero también se atenúan bordes y estructuras pequeñas.

Para mantener una evaluación consistente, se analizan cuatro configuraciones discretas disponibles en la herramienta. El comportamiento observado para cada tamaño es el siguiente:
\begin{itemize}
    \item \textbf{$2 \times 2$:} corresponde a una compresión leve; preserva la mayor parte del detalle fino y mantiene visibles bordes vasculares y exudados de pequeña escala.
    \item \textbf{$4 \times 4$:} ofrece un nivel intermedio de simplificación; reduce variaciones locales sin deteriorar de forma crítica la interpretación global de la escena.
    \item \textbf{$8 \times 8$:} prioriza la visualización de patrones globales de intensidad; como contrapartida, disminuye la nitidez de estructuras pequeñas.
    \item \textbf{$16 \times 16$:} produce una simplificación agresiva; resulta útil para análisis macroscópico, pero puede suprimir información clínica relevante de tamaño reducido.
\end{itemize}
En consecuencia, la selección del tamaño de bloque debe responder al objetivo analítico: valores pequeños cuando se requiere preservar detalle diagnóstico y valores mayores cuando se prioriza la compactación de datos o la observación de tendencias globales.
